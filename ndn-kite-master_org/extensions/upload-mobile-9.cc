...
#include "upload-mobile-9.h"
#include <sstream>



NS_LOG_COMPONENT_DEFINE ("MobileApp9");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (MobileApp9);


MobileApp9::MobileApp9 ()
{
  m_face = 0;
}

TypeId
MobileApp9::GetTypeId ()
{
  static TypeId tid = TypeId ("MobileApp9")
    .SetParent<Application> ()
    .AddConstructor<MobileApp9> ()
    .AddAttribute ("ServerPrefix", "The name prefix of server.",
                  StringValue ("/server9/upload/mobile9/file"),
                  MakeStringAccessor (&MobileApp9::m_serverPrefix),
                  MakeStringChecker ())
    .AddAttribute ("MobilePrefix", "The name prefix of mobile.",
                  StringValue ("/mobile9/file"),
                  MakeStringAccessor (&MobileApp9::m_mobilePrefix),
                  MakeStringChecker ())
    .AddAttribute ("RequestPeriod", "The time interval of sending request Interest.",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&MobileApp9::m_requestPeriod),
...
void
MobileApp9::StartApplication ()
{
  NS_LOG_FUNCTION (this);

  m_face = CreateObject<ApiFace> (GetNode ());
  Ptr<ndn::Name> prefix = Create<ndn::Name> (m_mobilePrefix);
  m_face->SetInterestFilter (prefix, MakeCallback (&MobileApp9::OnInterest, this));
  SendInterest ();
}

void
MobileApp9::StopApplication ()
{
  m_face->Shutdown ();
  m_face = 0;
}
...
void
MobileApp9::OnInterest (Ptr<const ndn::Name> origName, Ptr<const ndn::Interest> interest)
{
	...
	oss<< "4 MobileApp9::OnInterest::on_receive_interest: " << now.GetMilliSeconds()<<" , ";
	...
	oss1<< "5 MobileApp9::OnInterest:before_put_data: " << now.GetMilliSeconds()<<" , ";
...
void
MobileApp9::OnData (Ptr<const ndn::Interest> origInterest, Ptr<const ndn::Data> data)
{
  NS_LOG_FUNCTION (data->GetNamePtr ()); 
}

void
MobileApp9::OnTimeout (Ptr<const ndn::Interest> interest)
{
  SendInterest ();
  return;
}
...
MobileApp9::SendInterest ()
{
  ...
  std::ostringstream oss;
  oss<< "1 MobileApp9::SendInterest: " << now.GetMilliSeconds()<<" , ";
  std::string tsLog(oss.str());
  Log::write_ts_to_log_file(tsLog);
  oss.flush();
  ...
  std::ostringstream oss1;
  	...
  	std::string tsLog1(oss1.str());
  	Log::write_to_traced_interest_tracker_node_n(tsLog1, "/home/vartika-kite/kite_log_traced_9.txt");
  	oss.flush();

  Simulator::ScheduleNow (&ApiFace::ExpressInterest, m_face, interest,
                     MakeCallback (&MobileApp9::OnData, this),
                     MakeCallback (&MobileApp9::OnTimeout, this));
