...
#include "upload-server-2.h"
#include <sstream>

NS_LOG_COMPONENT_DEFINE ("ServerApp2");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (ServerApp2);


ServerApp2::ServerApp2 ()
{
  m_face = 0;
}

TypeId
ServerApp2::GetTypeId ()
{
  static TypeId tid = TypeId ("ServerApp2")
    .SetParent<Application> ()
    .AddConstructor<ServerApp2> ()
    .AddAttribute ("ServerPrefix", "The name prefix of server.",
                  StringValue ("/server2/upload"),
                  MakeStringAccessor (&ServerApp2::m_serverPrefix),
                  MakeStringChecker ())
    .AddAttribute ("RequestPeriod", "The time interval of sending request Interest.",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&ServerApp2::m_requestPeriod),
...
void
ServerApp2::StartApplication ()
{
  NS_LOG_FUNCTION ("vartika"<<this<<m_serverPrefix);
	// NS_LOG_FUNCTION (this);
  m_face = CreateObject<ApiFace> (GetNode ());
  Ptr<ndn::Name> prefix = Create<ndn::Name> (m_serverPrefix);
  m_face->SetInterestFilter (prefix, MakeCallback (&ServerApp2::OnInterest, this));
...
void
ServerApp2::OnInterest (Ptr<const ndn::Name> origName, Ptr<const ndn::Interest> interest)
{
//	NS_LOG_FUNCTION (interest);
	...
	oss<< "2 ServerApp2::OnInterest: " << now.GetMilliSeconds()<<" , ";
...
void
ServerApp2::OnData (Ptr<const ndn::Interest> origInterest, Ptr<const ndn::Data> data)
{
	...
	//20150924
	std::ostringstream oss;
	oss<< "6 ServerApp2::OnData: " << now.GetMilliSeconds() <<" , ";//std::endl;
	std::string tsLog(oss.str());
	Log::write_ts_to_log_file(tsLog);
	oss.flush();
...
void
ServerApp2::OnTimeout (Ptr<const ndn::Interest> interest)
{
  Ptr<const ndn::Name> name = interest->GetNamePtr ();
...
void
ServerApp2::SendInterest (int seq)
{
	...
	std::string tsLog1(oss1.str());
	Log::write_to_tracing_interest_tracker_node_n(tsLog1, "/home/vartika-kite/kite_log_tracing_2.txt");
	oss1.flush();


	Simulator::Schedule (Seconds (0), &ApiFace::ExpressInterest, m_face, interest,
					 MakeCallback (&ServerApp2::OnData, this),
					 MakeCallback (&ServerApp2::OnTimeout, this));
