...
#include "upload-mobile-2.h"
#include <sstream>



NS_LOG_COMPONENT_DEFINE ("MobileApp2");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (MobileApp2);
...
TypeId
MobileApp2::GetTypeId ()
{
  static TypeId tid = TypeId ("MobileApp2")
    .SetParent<Application> ()
    .AddConstructor<MobileApp2> ()
    .AddAttribute ("ServerPrefix", "The name prefix of server.",
                  StringValue ("/server2/upload/mobile2/file"),
                  MakeStringAccessor (&MobileApp2::m_serverPrefix),
                  MakeStringChecker ())
    .AddAttribute ("MobilePrefix", "The name prefix of mobile.",
                  StringValue ("/mobile2/file"),
                  MakeStringAccessor (&MobileApp2::m_mobilePrefix),
                  MakeStringChecker ())
    .AddAttribute ("RequestPeriod", "The time interval of sending request Interest.",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&MobileApp2::m_requestPeriod),
...
void
MobileApp2::StartApplication ()
{
  NS_LOG_FUNCTION (this<<m_mobilePrefix);

  m_face = CreateObject<ApiFace> (GetNode ());
  Ptr<ndn::Name> prefix = Create<ndn::Name> (m_mobilePrefix);
  m_face->SetInterestFilter (prefix, MakeCallback (&MobileApp2::OnInterest, this));
  SendInterest ();
}

void
MobileApp2::StopApplication ()
{
  m_face->Shutdown ();
  m_face = 0;
}
...
void
MobileApp2::OnInterest (Ptr<const ndn::Name> origName, Ptr<const ndn::Interest> interest)
{
	...
	oss<< "4 MobileApp2::OnInterest::on_receive_interest: " << now.GetMilliSeconds()<<" , ";
	...
	oss1<< "5 MobileApp2::OnInterest:before_put_data: " << now.GetMilliSeconds()<<" , ";
...
void
MobileApp2::OnData (Ptr<const ndn::Interest> origInterest, Ptr<const ndn::Data> data)
{
  NS_LOG_FUNCTION (data->GetNamePtr ()); 
}

void
MobileApp2::OnTimeout (Ptr<const ndn::Interest> interest)
{
  SendInterest ();
  return;
}
...
MobileApp2::SendInterest ()
{
  ...
  std::ostringstream oss;
  oss<< "1 MobileApp2::SendInterest: " << now.GetMilliSeconds()<<" , ";
  std::string tsLog(oss.str());
  Log::write_ts_to_log_file(tsLog);
  oss.flush();
  ...
  std::string tsLog1(oss1.str());
  Log::write_to_traced_interest_tracker_node_n(tsLog1, "/home/vartika-kite/kite_log_traced_2.txt");
  oss.flush();

  //NS_LOG_FUNCTION ("vartika interest name: "<<m_serverPrefix);

  Simulator::ScheduleNow (&ApiFace::ExpressInterest, m_face, interest,
                     MakeCallback (&MobileApp2::OnData, this),
                     MakeCallback (&MobileApp2::OnTimeout, this));
