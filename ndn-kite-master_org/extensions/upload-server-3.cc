...
#include "upload-server-3.h"
#include <sstream>

NS_LOG_COMPONENT_DEFINE ("ServerApp3");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (ServerApp3);


ServerApp3::ServerApp3 ()
{
  m_face = 0;
}

TypeId
ServerApp3::GetTypeId ()
{
  static TypeId tid = TypeId ("ServerApp3")
    .SetParent<Application> ()
    .AddConstructor<ServerApp3> ()
    .AddAttribute ("ServerPrefix", "The name prefix of server.",
                  StringValue ("/server3/upload"),
                  MakeStringAccessor (&ServerApp3::m_serverPrefix),
                  MakeStringChecker ())
    .AddAttribute ("RequestPeriod", "The time interval of sending request Interest.",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&ServerApp3::m_requestPeriod),
...
void
ServerApp3::OnInterest (Ptr<const ndn::Name> origName, Ptr<const ndn::Interest> interest)
{
	...
	oss<< "2 ServerApp3::OnInterest: " << now.GetMilliSeconds()<<" , ";
...
void
ServerApp3::OnData (Ptr<const ndn::Interest> origInterest, Ptr<const ndn::Data> data)
{
	...
	//20150924
	std::ostringstream oss;
	oss<< "6 ServerApp3::OnData: " << now.GetMilliSeconds() <<" , ";//std::endl;
	std::string tsLog(oss.str());
	Log::write_ts_to_log_file(tsLog);
	oss.flush();
	...
	Log::write_to_on_data_tracker(tsLog1, "/home/vartika-kite/kite_log_data_3.txt");
	oss1.flush();
...
void
ServerApp3::OnTimeout (Ptr<const ndn::Interest> interest)
{
  Ptr<const ndn::Name> name = interest->GetNamePtr ();
...
void
ServerApp3::SendInterest (int seq)
{
	...
	std::string tsLog1(oss1.str());
	Log::write_to_tracing_interest_tracker_node_n(tsLog1, "/home/vartika-kite/kite_log_tracing_3.txt");
	oss1.flush();


	Simulator::Schedule (Seconds (0), &ApiFace::ExpressInterest, m_face, interest,
					 MakeCallback (&ServerApp3::OnData, this),
					 MakeCallback (&ServerApp3::OnTimeout, this));
