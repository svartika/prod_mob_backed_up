...
main (int argc, char *argv[])
{
  ...
  std::string syncPrefix8 = "/server8/upload";
  Ptr<Node> rpNode8 = grid.GetNode (1, 3);

  std::string syncPrefix9 = "/server9/upload";
  Ptr<Node> rpNode9 = grid.GetNode (0, 2);
  ...
  for(uint32_t nodeCount=0; nodeCount<mobileNodes.GetN(); ++nodeCount)
   {
 		   ...
 		   i++)
 	  {
		...
		//} else if(nodeCount==7) {
			//vartika 20150920
			ndn::StackHelper::AddRoute (face->GetNode (), syncPrefix8, face->GetId (), 1);
		//} else if(nodeCount==8) {
			//vartika 20150920
			ndn::StackHelper::AddRoute (face->GetNode (), syncPrefix9, face->GetId (), 1);
		//}
 	  }
   }
  ...
  ndn::AppHelper rpHelper2 ("ServerApp2");
   ...
   ndn::AppHelper rpHelper8 ("ServerApp8");
   ApplicationContainer rpApp8 = rpHelper8.Install (rpNode8);
   rpApp8.Start (Seconds (0));

   ndn::AppHelper appHelper8 ("MobileApp8");
   ApplicationContainer app8 = appHelper8.Install (mobileNodes.Get(7)); //mn1);//mobileNodes.Get(2));
   app8.Start (Seconds (1));

   ndn::AppHelper rpHelper9 ("ServerApp9");
   ApplicationContainer rpApp9 = rpHelper9.Install (rpNode9);
   rpApp9.Start (Seconds (0));

   ndn::AppHelper appHelper9 ("MobileApp9");
   ApplicationContainer app9 = appHelper9.Install (mobileNodes.Get(8)); //mn1);//mobileNodes.Get(2));
   app9.Start (Seconds (1));

   NS_LOG_FUNCTION ("vartika" <<"mn1, mn2, mn3, mn4, mn5, mn6, mn7, mn8, mn9 "<<mobileNodes.Get(0)
		   <<mobileNodes.Get(1)<<mobileNodes.Get(2)<<mobileNodes.Get(3)
		   <<mobileNodes.Get(4)<<mobileNodes.Get(5)<<mobileNodes.Get(6)
		   <<mobileNodes.Get(7)<<mobileNodes.Get(8));
