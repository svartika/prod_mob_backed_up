...
#include "upload-mobile-3.h"
#include <sstream>



NS_LOG_COMPONENT_DEFINE ("MobileApp3");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (MobileApp3);
...
TypeId
MobileApp3::GetTypeId ()
{
  static TypeId tid = TypeId ("MobileApp3")
    .SetParent<Application> ()
    .AddConstructor<MobileApp3> ()
    .AddAttribute ("ServerPrefix", "The name prefix of server.",
                  StringValue ("/server3/upload/mobile3/file"),
                  MakeStringAccessor (&MobileApp3::m_serverPrefix),
                  MakeStringChecker ())
    .AddAttribute ("MobilePrefix", "The name prefix of mobile.",
                  StringValue ("/mobile3/file"),
                  MakeStringAccessor (&MobileApp3::m_mobilePrefix),
                  MakeStringChecker ())
    .AddAttribute ("RequestPeriod", "The time interval of sending request Interest.",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&MobileApp3::m_requestPeriod),
...
void
MobileApp3::StartApplication ()
{
  NS_LOG_FUNCTION (this);

  m_face = CreateObject<ApiFace> (GetNode ());
  Ptr<ndn::Name> prefix = Create<ndn::Name> (m_mobilePrefix);
  m_face->SetInterestFilter (prefix, MakeCallback (&MobileApp3::OnInterest, this));
  SendInterest ();
}

void
MobileApp3::StopApplication ()
{
  m_face->Shutdown ();
  m_face = 0;
}
...
void
MobileApp3::OnInterest (Ptr<const ndn::Name> origName, Ptr<const ndn::Interest> interest)
{
	...
	oss<< "4 MobileApp3::OnInterest::on_receive_interest: " << now.GetMilliSeconds()<<" , ";
	...
	oss1<< "5 MobileApp3::OnInterest:before_put_data: " << now.GetMilliSeconds()<<" , ";
...
void
MobileApp3::OnData (Ptr<const ndn::Interest> origInterest, Ptr<const ndn::Data> data)
{
  NS_LOG_FUNCTION (data->GetNamePtr ()); 
}

void
MobileApp3::OnTimeout (Ptr<const ndn::Interest> interest)
{
  SendInterest ();
  return;
}
...
MobileApp3::SendInterest ()
{
  ...
  std::ostringstream oss;
  oss<< "1 MobileApp3::SendInterest: " << now.GetMilliSeconds()<<" , ";
  std::string tsLog(oss.str());
  Log::write_ts_to_log_file(tsLog);
  oss.flush();
  ...
  std::ostringstream oss1;
  	...
  	std::string tsLog1(oss1.str());
  	Log::write_to_traced_interest_tracker_node_n(tsLog1, "/home/vartika-kite/kite_log_traced_3.txt");
  	oss.flush();

  Simulator::ScheduleNow (&ApiFace::ExpressInterest, m_face, interest,
                     MakeCallback (&MobileApp3::OnData, this),
                     MakeCallback (&MobileApp3::OnTimeout, this));
