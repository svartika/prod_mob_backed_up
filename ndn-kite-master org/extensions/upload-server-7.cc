...
#include "upload-server-7.h"
#include <sstream>

NS_LOG_COMPONENT_DEFINE ("ServerApp7");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (ServerApp7);


ServerApp7::ServerApp7 ()
{
  m_face = 0;
}

TypeId
ServerApp7::GetTypeId ()
{
  static TypeId tid = TypeId ("ServerApp7")
    .SetParent<Application> ()
    .AddConstructor<ServerApp7> ()
    .AddAttribute ("ServerPrefix", "The name prefix of server.",
                  StringValue ("/server7/upload"),
                  MakeStringAccessor (&ServerApp7::m_serverPrefix),
                  MakeStringChecker ())
    .AddAttribute ("RequestPeriod", "The time interval of sending request Interest.",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&ServerApp7::m_requestPeriod),
...
void
ServerApp7::OnInterest (Ptr<const ndn::Name> origName, Ptr<const ndn::Interest> interest)
{
	...
	oss<< "2 ServerApp7::OnInterest: " << now.GetMilliSeconds()<<" , ";
...
void
ServerApp7::OnData (Ptr<const ndn::Interest> origInterest, Ptr<const ndn::Data> data)
{
	...
	//20150924
	std::ostringstream oss;
	oss<< "6 ServerApp7::OnData: " << now.GetMilliSeconds() <<" , ";//std::endl;
	std::string tsLog(oss.str());
	Log::write_ts_to_log_file(tsLog);
	oss.flush();
	...
	Log::write_to_on_data_tracker(tsLog1, "/home/vartika-kite/kite_log_data_7.txt");
	oss1.flush();
...
void
ServerApp7::OnTimeout (Ptr<const ndn::Interest> interest)
{
  Ptr<const ndn::Name> name = interest->GetNamePtr ();
...
void
ServerApp7::SendInterest (int seq)
{
	...
	std::string tsLog1(oss1.str());
	Log::write_to_tracing_interest_tracker_node_n(tsLog1, "/home/vartika-kite/kite_log_tracing_7.txt");
	oss1.flush();


	Simulator::Schedule (Seconds (0), &ApiFace::ExpressInterest, m_face, interest,
					 MakeCallback (&ServerApp7::OnData, this),
					 MakeCallback (&ServerApp7::OnTimeout, this));
