...
void AnchorPointForwarding::OnInterest (Ptr<Face> inFace, Ptr<Interest> interest)
{
  ...
  if (pitEntry == 0) // vartika says - if no pit entry
    {
	  //whether to create PIT entry here or not?
	  //if i don't create then always interest will be sent to anchor
	  //if i do create then interest will not get response and die (interest = org name)
	  //this is because i will make new interest to anchor which will contain this interest packet. ( interest = anchor + org name)

	  //at anchor extract original interest packet
	  //then forward it to -- what?
	  //data comes back to anchor
	  //anchor will change this interest packet to send back to requesting node

	  //at that node..it will convert this anchor+org name to org name and create data attachment here
