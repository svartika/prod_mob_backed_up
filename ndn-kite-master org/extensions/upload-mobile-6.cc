...
#include "upload-mobile-6.h"
#include <sstream>



NS_LOG_COMPONENT_DEFINE ("MobileApp6");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (MobileApp6);
...
TypeId
MobileApp6::GetTypeId ()
{
  static TypeId tid = TypeId ("MobileApp6")
    .SetParent<Application> ()
    .AddConstructor<MobileApp6> ()
    .AddAttribute ("ServerPrefix", "The name prefix of server.",
                  StringValue ("/server6/upload/mobile6/file"),
                  MakeStringAccessor (&MobileApp6::m_serverPrefix),
                  MakeStringChecker ())
    .AddAttribute ("MobilePrefix", "The name prefix of mobile.",
                  StringValue ("/mobile6/file"),
                  MakeStringAccessor (&MobileApp6::m_mobilePrefix),
                  MakeStringChecker ())
    .AddAttribute ("RequestPeriod", "The time interval of sending request Interest.",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&MobileApp6::m_requestPeriod),
...
void
MobileApp6::StartApplication ()
{
  NS_LOG_FUNCTION (this);

  m_face = CreateObject<ApiFace> (GetNode ());
  Ptr<ndn::Name> prefix = Create<ndn::Name> (m_mobilePrefix);
  m_face->SetInterestFilter (prefix, MakeCallback (&MobileApp6::OnInterest, this));
  SendInterest ();
}

void
MobileApp6::StopApplication ()
{
  m_face->Shutdown ();
  m_face = 0;
}
...
void
MobileApp6::OnInterest (Ptr<const ndn::Name> origName, Ptr<const ndn::Interest> interest)
{
	...
	oss<< "4 MobileApp6::OnInterest::on_receive_interest: " << now.GetMilliSeconds()<<" , ";
	...
	oss1<< "5 MobileApp6::OnInterest:before_put_data: " << now.GetMilliSeconds()<<" , ";
...
void
MobileApp6::OnData (Ptr<const ndn::Interest> origInterest, Ptr<const ndn::Data> data)
{
  NS_LOG_FUNCTION (data->GetNamePtr ()); 
}

void
MobileApp6::OnTimeout (Ptr<const ndn::Interest> interest)
{
  SendInterest ();
  return;
}
...
MobileApp6::SendInterest ()
{
  ...
  std::ostringstream oss;
  oss<< "1 MobileApp6::SendInterest: " << now.GetMilliSeconds()<<" , ";
  std::string tsLog(oss.str());
  Log::write_ts_to_log_file(tsLog);
  oss.flush();
  ...
  std::ostringstream oss1;
  	...
  	std::string tsLog1(oss1.str());
  	Log::write_to_traced_interest_tracker_node_n(tsLog1, "/home/vartika-kite/kite_log_traced_6.txt");
  	oss.flush();

  Simulator::ScheduleNow (&ApiFace::ExpressInterest, m_face, interest,
                     MakeCallback (&MobileApp6::OnData, this),
                     MakeCallback (&MobileApp6::OnTimeout, this));
