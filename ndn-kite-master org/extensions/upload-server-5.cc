...
#include "upload-server-5.h"
#include <sstream>

NS_LOG_COMPONENT_DEFINE ("ServerApp5");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (ServerApp5);


ServerApp5::ServerApp5 ()
{
  m_face = 0;
}

TypeId
ServerApp5::GetTypeId ()
{
  static TypeId tid = TypeId ("ServerApp5")
    .SetParent<Application> ()
    .AddConstructor<ServerApp5> ()
    .AddAttribute ("ServerPrefix", "The name prefix of server.",
                  StringValue ("/server5/upload"),
                  MakeStringAccessor (&ServerApp5::m_serverPrefix),
                  MakeStringChecker ())
    .AddAttribute ("RequestPeriod", "The time interval of sending request Interest.",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&ServerApp5::m_requestPeriod),
...
void
ServerApp5::OnInterest (Ptr<const ndn::Name> origName, Ptr<const ndn::Interest> interest)
{
	...
	oss<< "2 ServerApp5::OnInterest: " << now.GetMilliSeconds()<<" , ";
...
void
ServerApp5::OnData (Ptr<const ndn::Interest> origInterest, Ptr<const ndn::Data> data)
{
	...
	//20150924
	std::ostringstream oss;
	oss<< "6 ServerApp5::OnData: " << now.GetMilliSeconds() <<" , ";//std::endl;
	std::string tsLog(oss.str());
	Log::write_ts_to_log_file(tsLog);
	oss.flush();
	...
	Log::write_to_on_data_tracker(tsLog1, "/home/vartika-kite/kite_log_data_5.txt");
	oss1.flush();
...
void
ServerApp5::OnTimeout (Ptr<const ndn::Interest> interest)
{
  Ptr<const ndn::Name> name = interest->GetNamePtr ();
...
void
ServerApp5::SendInterest (int seq)
{
	...
	std::string tsLog1(oss1.str());
	Log::write_to_tracing_interest_tracker_node_n(tsLog1, "/home/vartika-kite/kite_log_tracing_5.txt");
	oss1.flush();


	Simulator::Schedule (Seconds (0), &ApiFace::ExpressInterest, m_face, interest,
					 MakeCallback (&ServerApp5::OnData, this),
					 MakeCallback (&ServerApp5::OnTimeout, this));
